...
__global__ void unsigned_distance_backward_kernel(uint32_t n_elements, const float* __restrict__ grad_dist, const Vector3f* __restrict__ positions, const float* __restrict__ distances, const int64_t* __restrict__ face_id, Vector3f* __restrict__ grad_positions, const Triangle* __restrict__ triangles_unsorted);
...
public:
    ...
    void signed_distance_gpu(uint32_t n_elements, uint32_t mode, const float* positions, float* distances, int64_t* face_id, float* uvw, const Triangle* gpu_triangles, cudaStream_t stream) override {
        ...
        if (mode == 0) {
            linear_kernel(signed_distance_watertight_kernel, 0u, stream,
                n_elements, positions_vec, distances, face_id, uvw_vec, m_nodes_gpu.data(), gpu_triangles, false
            );
    ...
    void unsigned_distance_backward_gpu(uint32_t n_elements, const float* grad_dist, const float* positions, const float* distances, const int64_t* face_id, float* grad_positions, const Triangle* triangles_unsorted, cudaStream_t stream) override {
        const Vector3f* positions_vec = (const Vector3f*)positions;
        Vector3f* grad_positions_vec = (Vector3f*)grad_positions;

        linear_kernel(unsigned_distance_backward_kernel, 0u, stream,
            n_elements, grad_dist, positions_vec, distances, face_id, grad_positions_vec, triangles_unsorted
        );
    }
    ...
    void ray_trace_gpu(uint32_t n_elements, const float* rays_o, const float* rays_d, float* positions, int64_t* face_id, float* depth, const Triangle* gpu_triangles, cudaStream_t stream) override {
        ...
        linear_kernel(raytrace_kernel, 0u, stream,
            n_elements, rays_o_vec, rays_d_vec, positions_vec, face_id, depth, m_nodes_gpu.data(), gpu_triangles
        );
    }
...
__global__ void unsigned_distance_backward_kernel(
    uint32_t n_elements, const float* __restrict__ grad_dist, const Vector3f* __restrict__ positions,
    const float* __restrict__ distances, const int64_t* __restrict__ face_id, Vector3f* __restrict__ grad_positions,
    const Triangle* __restrict__ triangles_unsorted
) {
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n_elements) return;
    ...
    if (dist < 1e-6) {
        grad_positions[i] = {0.0f, 0.0f, 0.0f};
        return;
    }

    Vector3f point = positions[i];
    const Triangle& tri = triangles_unsorted[face_id[i]];
    
    Vector3f closest_point = tri.closest_point(point);
    Vector3f grad = (point - closest_point) / dist;
    
    grad_positions[i] = grad * grad_dist[i];
}
